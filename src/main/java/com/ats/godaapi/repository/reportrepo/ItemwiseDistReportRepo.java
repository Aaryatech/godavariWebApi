...
public interface ItemwiseDistReportRepo extends JpaRepository<ItemwiseDistReport, Integer> {
	...
	@Query(value = "SELECT t_order_detail.order_detail_id,m_item.item_eng_name,m_item.item_mar_name,m_item.item_wt,"
			...
			+ "	 AND t_order_detail.order_header_id=t_order_header.order_header_id AND t_order_header.order_date "
			+ "	BETWEEN :fromDate AND :toDate"
			+ "	 AND t_order_header.dist_id AND t_order_header.order_status=:orderStatus "
			+ "AND t_order_header.dist_id=m_dist.dist_id"
			+ "	GROUP BY m_item.item_id, t_order_header.dist_id ", nativeQuery = true)
	List<ItemwiseDistReport> getAllItemwiseDistReport(@Param("fromDate") String fromDate,
	...
	@Query(value = "SELECT t_order_detail.order_detail_id,m_item.item_eng_name,m_item.item_mar_name,m_item.item_wt,"
			...
			+ "	 AND t_order_detail.order_header_id=t_order_header.order_header_id AND t_order_header.order_date "
			+ "	BETWEEN :fromDate AND :toDate"
			+ "	 AND  t_order_header.order_status=:orderStatus "
			+ "AND t_order_header.dist_id=m_dist.dist_id AND m_dist.hub_id IN (:hubIdList)"
			+ "	GROUP BY m_item.item_id,m_dist.hub_id ", nativeQuery = true)
	List<ItemwiseDistReport> getItemwiseDistReportForHubs(@Param("fromDate") String fromDate, @Param("toDate") String toDate,
			@Param("hubIdList") List<Integer> hubIdList, @Param("orderStatus") int orderStatus);
	...
	@Query(value = "SELECT t_order_detail.order_detail_id,m_item.item_eng_name,m_item.item_mar_name,m_item.item_wt,"
			...
			+ "AND t_order_header.dist_id=m_dist.dist_id "
			+ "	GROUP BY m_item.item_id,m_dist.hub_id ", nativeQuery = true)
	List<ItemwiseDistReport> getAllItemwiseDistReportForHubs(@Param("fromDate") String fromDate, @Param("toDate") String toDate, @Param("orderStatus") int orderStatus);
