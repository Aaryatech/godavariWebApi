...
public interface SettingRepo extends JpaRepository<Setting, Integer> {
	...
	@Query(value = "SELECT s.* FROM  t_setting s,m_dist d,t_config c  WHERE d.hub_id=s.hub_id  AND s.config_id=c.config_id AND TIME(:currentTime) >= TIME(c.hub_from_time) AND TIME(:currentTime) <=TIME(c.hub_to_time)", nativeQuery = true)
