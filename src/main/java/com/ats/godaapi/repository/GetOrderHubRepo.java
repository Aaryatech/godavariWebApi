...
public interface GetOrderHubRepo extends JpaRepository<GetOrderHub, Integer> {
	...
	@Query(value = "SELECT h.*,d.dist_eng_name,d.dist_mar_name,d.dist_add_mar,d.dist_add_eng ,d.dist_contact_no FROM t_order_header h,m_dist d WHERE  d.dist_id=h.dist_id AND h.order_status IN (:orderStatusList) AND h.order_date=:currDate", nativeQuery = true)
	List<GetOrderHub> getOrderHub(@Param("currDate") String currDate,@Param("orderStatusList") List<Integer> orderStatusList);
	...
	@Query(value = "SELECT h.*, d.dist_eng_name, d.dist_mar_name, d.dist_add_mar, d.dist_add_eng , d.dist_contact_no FROM t_order_header h, m_dist d WHERE d.hub_id=:hubId and  d.dist_id=h.dist_id  AND h.order_type IN (:orderType) AND h.order_status=:orderStatus AND h.order_date=:currDate", nativeQuery = true)
